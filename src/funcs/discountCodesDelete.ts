 ...
 */
export function discountCodesDelete(
  client: DubCore,
  idOrCode: string,
...
> {
  const input: operations.DeleteDiscountCodeRequest = {
    idOrCode: idOrCode,
  };

  const parsed = safeParse(
    input,
    (value) => operations.DeleteDiscountCodeRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    idOrCode: encodeSimple("idOrCode", payload.idOrCode, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  const path = pathToFunc("/discount-codes/{idOrCode}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.DeleteDiscountCodeResponseBody$inboundSchema),
